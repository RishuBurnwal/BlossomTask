...
REVERIFY_DEFAULT_PROVIDER = os.getenv("REVERIFY_DEFAULT_PROVIDER", "openai")
...
def _partial_timing_note(
    ...
    dates_present = any(_safe_str(value) for value in [service_date, visitation_date, delivery_date, ceremony_date])
    times_present = any(_safe_str(value) for value in [service_time, visitation_time, delivery_time, ceremony_time])
    has_datetime_pair = bool(
        (service_date and service_time)
        or (visitation_date and visitation_time)
        or (delivery_date and delivery_time)
...
def _collect_reference_dates(record: dict) -> list[str]:
    values = []
    values.extend(_extract_dates_from_text(record.get("ord_instruct")))
    for key in ("delivery_date", "ship_delivery_date", "service_date", "ord_date", "order_date"):
        normalized = _parse_date_candidate(record.get(key))
        if normalized:
            values.append(normalized)
    return _unique_non_empty(values)


def _evaluate_source_freshness(record: dict, parsed: dict) -> tuple[str, str]:
    ...
    reference_dates = _collect_reference_dates(record)
...
def _evaluate_venue_alignment(record: dict, parsed: dict) -> tuple[str, str]:
    expected = _safe_str(record.get("ship_care_of"))
...
def apply_business_rules(record: dict, parsed: dict) -> dict:
    ...
    has_external_source_evidence = bool(source_urls or original_venue_evidence)
    ...
    source_freshness_status, source_freshness_note = _evaluate_source_freshness(record, adjusted)
    adjusted["source_freshness_status"] = source_freshness_status
    notes = append_note(notes, source_freshness_note)
    venue_alignment_status, venue_alignment_note = _evaluate_venue_alignment(record, adjusted)
    adjusted["venue_alignment_status"] = venue_alignment_status
    notes = append_note(notes, venue_alignment_note)
    ...
    if name_match_status == "mismatch" and (has_source_evidence or has_any_timing or instruction_has_schedule):
        ...
        adjusted["match_status"] = "Review"
        adjusted["ai_accuracy_score"] = min(max(score, 60.0), 84.0)
        notes = append_note(notes, "Review: source venue conflicts with recipient / care-of details")
        ...
        notes = append_note(notes, "Review: source timing is stale or backdated for this order")
        ...
        notes = append_note(notes, "Customer: schedule normalized from order instructions because no trustworthy obituary/detail source confirmed it")
    elif (
        date_verification_status in {"verified", "source_only"}
        and name_match_status in {"exact", "minor", "fuzzy"}
        and (has_strong_source_url or (date_verification_status == "source_only" and original_venue_evidence))
